...
- `src/lib/talent-api.ts` — EVO-Talent's typed client: goals, appraisals, competency ratings, courses/enrollments, career paths, succession plans, `getEmployeeScore`, `getTalentDashboardSummary` — hitting `/talent/...`. IT-Assets has no API client yet — there's no backend for it.
