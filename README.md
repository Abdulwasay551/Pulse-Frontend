...
- `src/lib/people-api.ts` — EVO-People's typed client: employees CRUD + dashboard-summary, employee documents (upload/delete), `employeesCsv`, attendance records, shifts, leave requests, surveys/survey responses, recognitions, promotion requests — hitting `/people/...`. Talent/IT-Assets have no API client yet — there's no backend for them.
