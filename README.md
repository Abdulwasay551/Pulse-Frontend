...
- **`(app)`** — `/login`, `/signup`, `/forgot-password`, `/reset-password`, `/dashboard/*`. Real auth (JWT via `src/lib/auth-context.tsx`) gates everything under `/dashboard` (`AuthGuard`). `/dashboard` itself is a **module hub** (hero + cards) for the five modules the product is expanding into:
  ...
  - `/dashboard/people` — **EVO-People Management** (HR core). Every feature from the spec is navigable; none has a backend yet, so every tile/sidebar link is "coming soon".
  ...
  **Every module's full feature taxonomy — every section and every feature from the Evo HRIS spec, real or not — lives in one place: `src/lib/dashboard-modules.ts`.** `Sidebar` renders it as collapsible per-section submenus (a section with only one feature collapses to a flat link), and each hub page renders the same data as tiles via `src/components/dashboard/ModuleFeatureSections.tsx` — so the sidebar and the hub page can never drift out of sync with each other. A feature is "real" if its entry has an `href`; otherwise `featureHref()` routes it to `/dashboard/coming-soon` with the feature/section/module name as query params, so **every single nav entry and tile is a real, clickable link** — nothing is a dead, unclickable placeholder anymore.
