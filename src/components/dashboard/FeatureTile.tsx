...
export default function FeatureTile({
  ...
  const content = (
    <div
      className={`group relative h-full rounded-2xl border border-line bg-card p-5 ${
        href ? "transition-all hover:-translate-y-0.5 hover:border-primary/40 hover:shadow-md" : ""
      }`}
    >
      {!href && (
        <span className="absolute top-4 right-4 rounded-full bg-cream-dim px-2 py-0.5 font-mono text-[10px] font-semibold whitespace-nowrap text-ink-soft">
          Coming soon
        </span>
      )}
