    ...
    <header className="sticky top-0 z-50 border-b border-line bg-cream/90 backdrop-blur">
      <nav className="mx-auto flex max-w-6xl items-center justify-between px-6 py-4">
        ...
        <div className="hidden items-center gap-8 md:flex">
          {navLinks.map((link) => (
            <Link
              key={link.label}
              href={link.href}
              className="font-sans text-sm font-medium text-ink-soft transition-colors hover:text-ink"
            >
              {link.label}
            </Link>
          ))}
        ...
        <div className="hidden items-center gap-3 md:flex">
          <Link
            href="/login"
            className="rounded-lg border border-line px-4 py-2 font-sans text-sm font-semibold text-ink transition-colors hover:bg-cream-dim"
          >
            Log in
          </Link>
          <Link
      ...
      {open && (
        <div className="flex flex-col gap-4 border-t border-line bg-cream px-6 py-6 md:hidden">
          ...
          <div className="mt-2 flex flex-col gap-3">
            <Link
              href="/login"
              className="rounded-lg border border-line px-4 py-2 text-center font-sans text-sm font-semibold text-ink"
              onClick={() => setOpen(false)}
            >
              Log in
            </Link>
