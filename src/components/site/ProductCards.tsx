...
const LG_HEIGHTS = ["lg:h-[400px]", "lg:h-[320px]", "lg:h-[370px]", "lg:h-[300px]", "lg:h-[430px]"];
        ...
        <div className="flex flex-col gap-5 lg:flex-row lg:items-end">
          {products.map((product, i) => (
            ...
            >
              ...
              <div
                className="flex h-full flex-col overflow-hidden border border-line bg-card p-6 transition-all duration-300 ease-out group-hover:-translate-y-1 group-hover:shadow-lg group-hover:shadow-ink/10"
                style={{
                  clipPath:
                    "polygon(28px 0, 100% 0, 100% 100%, 0 100%, 0 28px, 28px 28px)",
                }}
              >
                <ProductIcon
                ...
                />
                <span className="mb-3 inline-block w-fit font-mono text-xs font-semibold tracking-wide text-primary">
                  {product.tag}
                </span>
                <h3 className="font-display text-lg font-bold text-ink">{product.name}</h3>
                <p className="mt-2 text-sm text-ink-soft">{product.short_description}</p>

                <Link
                  href="/solutions"
                  className="mt-auto flex w-fit items-center gap-1.5 pt-5 font-mono text-xs font-semibold text-primary transition-colors hover:text-primary-dark"
                >
