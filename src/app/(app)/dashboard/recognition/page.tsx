...
import { employeesApi, recognitionsApi, type Employee, type Recognition } from "@/lib/people-api";
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      await withAuth((token) =>
        recognitionsApi.create(token, { employee: Number(form.employee), given_by: form.given_by, message: form.message })
      );
      setShowForm(false);
      await load();
    } catch (err) {
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {recognitions.map((r) => (
            <div key={r.id} className="group relative rounded-2xl border border-line bg-card p-5">
              ...
              <h3 className="font-display text-base font-bold text-ink">{r.employee_detail.name}</h3>
              <p className="mt-1.5 text-sm text-ink-soft">{r.message}</p>
