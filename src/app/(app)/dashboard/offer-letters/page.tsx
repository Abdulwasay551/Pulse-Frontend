          ...
          <tbody>
            {offers.map((o) => (
              <tr key={o.id} className="group border-b border-line last:border-0 hover:bg-cream/60">
                ...
                <td className="px-5 py-3.5 font-mono text-ink-soft">{o.salary ?? "—"}</td>
                <td className="px-5 py-3.5 font-mono text-xs text-ink-soft">{o.start_date ?? "—"}</td>
                <td className="px-5 py-3.5">
                  <span
                    className={`rounded-full px-2.5 py-1 font-mono text-[11px] font-semibold whitespace-nowrap ${statusTone[o.status]}`}
                  >
                    {o.status}
                  </span>
                </td>
